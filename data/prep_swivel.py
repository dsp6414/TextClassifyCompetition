...
flags.DEFINE_string('input', '../input/word.txt', 'The input text.')
flags.DEFINE_string('output_dir', '/home/xiaqiang/total_data/daguanbei/word_swivel_data',
...
def compute_coocs(lines, vocab):
    ...
    num_shards = int(len(vocab) / FLAGS.shard_size)
    ...
    def flush_coocs():
        for (row_id, col_id), cnt in coocs.items():
            ...
            row_off = int(row_id / num_shards)
            col_shard = col_id % num_shards
            col_off = int(col_id / num_shards)
